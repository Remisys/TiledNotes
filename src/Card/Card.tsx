...
function Card(props){
    ...
    return(
        <div className="rounded-lg bg-slate-50 p-2 border-2 border-sky-600 hover:scale-[1.02] flex flex-col " onMouseOut={() => setOnHover(false)} onMouseOver={() => setOnHover(true)} style={{gridRow: props.gridRow, gridColumn: props.gridColumn}}>
            ...
            <textarea className="appearance-none outline-none text-black w-[100%] grow mx-1"  disabled={!onHover} spellCheck="false" value={props.card.content} onChange={(e) => {pushContent((e.target as HTMLTextAreaElement).value)}}></textarea>
